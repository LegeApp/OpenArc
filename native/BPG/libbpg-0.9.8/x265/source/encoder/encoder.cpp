...
void Encoder::configureZone(x265_param *p, x265_param *zone)
{
    if (m_param->bResetZoneConfig)
    {
        ...
        {
            p->rc.qp = zone->rc.qp;
            /* OpenArc/BPG still-image encoding intentionally keeps AQ active
             * in CQP mode.  Upstream x265 disabled AQ here, which meant
             * libbpg's hard-coded "ssim" tune was silently erased. */
        }
...
void Encoder::configure(x265_param *p)
{
    ...
    if (p->rc.rateControlMode == X265_RC_CQP)
    {
        p->rc.bitrate = 0;
        p->rc.cuTree = 0;
        /* Do not clear aqMode/aqStrength here.  x265.h documents CQP as a
         * base QP that adaptive quantization may alter per block, and BPG's
         * SSIM-tuned still-image path depends on that behavior. */
    }
