...
impl EncryptionInfo {
    ...
    pub fn from_method_string(method: &str, crypto_flags: Option<&str>) -> Result<Self> {
        ...
        let (cipher_part, mode) = if main_part.contains('/') {
            ...
            (
                slash_parts[0],
                slash_parts.get(1).unwrap_or(&"ctr").to_string(),
            )
        } else {
...
impl PasswordDeriver {
    ...
    pub fn derive_key(
        ...
        pbkdf2_hmac::<Sha512>(password.as_bytes(), salt_bytes, self.iterations, &mut key);
    ...
    pub fn derive_iv(&self, password: &str, iv_len: usize) -> Result<Vec<u8>> {
        use pbkdf2::pbkdf2_hmac;
        use sha2::Sha512;
        ...
        pbkdf2_hmac::<Sha512>(password.as_bytes(), iv_salt, self.iterations, &mut iv);
...
impl CascadedDecryptor {
    ...
    fn verify_password(enc_info: &EncryptionInfo, password: &str) -> Result<bool> {
        ...
        pbkdf2_hmac::<Sha512>(&password_bytes, salt, enc_info.iterations, &mut derived);
    ...
    pub fn new(enc_info: &EncryptionInfo, password: &str) -> Result<Self> {
        if enc_info.algorithms.is_empty() || enc_info.algorithms[0] == CipherAlgorithm::None {
            return Ok(CascadedDecryptor { ciphers: vec![] });
        }
        ...
        pbkdf2_hmac::<Sha512>(&password_bytes, salt_bytes, enc_info.iterations, &mut key);
        ...
        let iv = enc_info
            .iv
            .as_ref()
            .ok_or_else(|| anyhow!("No IV provided in encryption parameters"))?;
            ...
            let cipher: Box<dyn CipherOp> = match algo {
                ...
                CipherAlgorithm::Blowfish => {
                    ...
                    let blowfish_iv = if iv.len() >= 8 {
                        ...
                        return Err(anyhow!(
                            "Blowfish requires 8-byte IV, got {} bytes",
                            iv.len()
                        ));
                    };

                    eprintln!(
                        "Blowfish decrypt: key_size={}, iv_len={}, iterations={}",
                        key.len(),
                        blowfish_iv.len(),
                        enc_info.iterations
                    );
                ...
                CipherAlgorithm::AES => {
                    ...
                    eprintln!(
                        "AES decrypt: key_size={}, iv_len={}, iterations={}",
                        key.len(),
                        iv.len(),
                        enc_info.iterations
                    );
                ...
                CipherAlgorithm::Twofish => {
                    ...
                    if iv.len() != 16 {
                        return Err(anyhow!(
                            "Twofish requires 16-byte IV, got {} bytes",
                            iv.len()
                        ));
                    }

                    eprintln!(
                        "Twofish decrypt: key_size={}, iv_len={}, iterations={}",
                        key.len(),
                        iv.len(),
                        enc_info.iterations
                    );
                ...
                CipherAlgorithm::Serpent => {
                    ...
                    if iv.len() != 16 {
                        return Err(anyhow!(
                            "Serpent requires 16-byte IV, got {} bytes",
                            iv.len()
                        ));
                    }

                    eprintln!(
                        "Serpent decrypt: key_size={}, iv_len={}, iterations={}",
                        key.len(),
                        iv.len(),
                        enc_info.iterations
                    );
...
pub fn create_encryptor(
    encryption_spec: &str,
    password: &str,
) -> Result<(String, CascadedDecryptor)> {
    let generator = match encryption_spec.to_lowercase().as_str() {
        ...
        _ => {
            return Err(anyhow!(
                "Unknown encryption method: {}. Supported: blowfish, aes-256, aes-128",
                encryption_spec
            ))
        }
    };
