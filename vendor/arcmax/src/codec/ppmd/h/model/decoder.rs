use super::super::{SYM_END, SYM_ERROR};
use super::*;
...
impl<R: Read> PPMd7<RangeDecoder<R>> {
    pub(crate) fn decode_symbol(&mut self) -> Result<i32, std::io::Error> {
        unsafe {
            ...
            let num_stats = min_context_ref.num_stats;

            if num_stats != 1 {
                let mut s = self.get_multi_state_stats(min_context);
                let summ_freq = min_context_ref.union2.summ_freq as u32;
                ...
                let s2 = self.get_multi_state_stats(min_context);
                Self::mask_symbols(&mut char_mask, s, s2);
            } else {
                let s = self.get_single_state(min_context);
                let symbol0 = s.as_ref().symbol;
