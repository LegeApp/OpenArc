...
const fn build_hi_bits_flag3() -> [u8; 256] {
    let mut table = [0; 256];
    let mut symbol = 0;
    while symbol < 256 {
        table[symbol] = (((symbol as u32 + 0xC0) >> 5) & 8) as u8;
        symbol += 1;
    }
    table
}

const fn build_hi_bits_flag4() -> [u8; 256] {
    let mut table = [0; 256];
    let mut symbol = 0;
    while symbol < 256 {
        table[symbol] = (((symbol as u32 + 0xC0) >> 4) & 16) as u8;
        symbol += 1;
    }
    table
}

static K_HI_BITS_FLAG3: [u8; 256] = build_hi_bits_flag3();
static K_HI_BITS_FLAG4: [u8; 256] = build_hi_bits_flag4();
...
impl<RC> PPMd7<RC> {
            ...
            *fresh.as_mut() = self.found_state.as_ref().symbol;
            ...
            while c != mc {
                ...
                if ns1 != 1 {
                    ...
                    // max(init_esc = self.exp_escape[*]) is 25. So the max(escape_freq) is 26 here.
    ...
    unsafe fn update_bin(&mut self, mut s: NonNull<State>) {
        unsafe {
            ...
            self.prev_success = 1;
            self.run_length += 1;
            state.freq = freq + (freq < 128) as u8;
            self.next_context();
        }
    }
    ...
    #[inline(always)]
    fn hi_bits_flag3(symbol: u32) -> u32 {
        K_HI_BITS_FLAG3[symbol as usize] as u32
    }

    #[inline(always)]
    unsafe fn get_bin_summ(&mut self) -> &mut u16 {
        unsafe {
            let min_context = self.min_context;
            let min_context_ref = min_context.as_ref();
            let state = {
                let single_state = &raw mut (*min_context.as_ptr()).union2;
                NonNull::new_unchecked(single_state).cast::<State>()
            };
            let state_ref = state.as_ref();

            let found_symbol = self.found_state.as_ref().symbol as usize;
            let symbol = state_ref.symbol as usize;
            let hi_bits_flag3 = K_HI_BITS_FLAG3[found_symbol] as u32;
            let hi_bits_flag4 = K_HI_BITS_FLAG4[symbol] as u32;

            self.hi_bits_flag = hi_bits_flag3;

            let freq_bin_idx = state_ref.freq as usize;

            let suffix_context = self.get_context(min_context_ref.suffix);
            let suffix_num_stats = suffix_context.as_ref().num_stats as usize;

            let context_idx = (self.prev_success
                + ((self.run_length as u32 >> 26) & 0x20)
                + self.ns2bs_index[suffix_num_stats - 1] as u32
                + hi_bits_flag4
