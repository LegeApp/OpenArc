use crate::core::crypto::{create_encryptor, CascadedDecryptor, EncryptionInfo};
use crate::formats::freearc::block::BlockDescriptor;
use crate::formats::freearc::constants::{BlockType, ARC_SIGNATURE};
use crate::formats::freearc::directory::{DataBlockInfo, DirectoryBlock, FileInfo};
...
impl<W: Write + Seek> FreeArcWriter<W> {
    ...
    fn compress_and_encrypt(&self, data: &[u8]) -> Result<(Vec<u8>, String)> {
        ...
        if method.starts_with("lzma") {
            ...
            processed = compress_lzma_compat(&processed, level)?;
    ...
    pub fn finish(mut self) -> Result<W> {
        ...
        for block in &mut self.data_blocks {
            block.offset = dir_start_pos
                .checked_sub(block.offset)
                .expect("Block pos > Dir pos?");
        }
...
fn compress_lzma_compat(data: &[u8], level: i32) -> Result<Vec<u8>> {
    if level > 0 {
        crate::codecs::lzma2::compress_lzma(data, level, 32 * 1024 * 1024, 3, 0, 2)
    } else {
        crate::codecs::lzma2::compress_lzma_default(data)
    }
}

#[cfg(not(feature = "ffi-codecs"))]
fn compress_lzma_compat(_data: &[u8], _level: i32) -> Result<Vec<u8>> {
    Err(anyhow!(
        "LZMA FreeArc archive writing requires the temporary ffi-codecs feature until native LZMA wiring is implemented"
    ))
}
