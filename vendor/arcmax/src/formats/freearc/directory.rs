...
impl DirectoryBlock {
    pub fn read<R: Read>(reader: &mut R, footer_pos: u64) -> Result<Self> {
        ...
        if reader.read(&mut tag_buf).is_ok() {
            if tag_buf[0] != 0 {
                ...
                eprintln!(
                    "Warning: Non-zero optional field tag encountered: {}",
                    tag_buf[0]
                );
            }
        }
        ...
        let mut files_in_current_block_remaining = if num_blocks > 0 {
            files_per_block[0]
        } else {
            0
        };
    ...
    pub fn write<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        // 2. Files per block
        let files_per_block: Vec<u64> = self
            .data_blocks
            .iter()
            .map(|b| b.num_files as u64)
            .collect();
        write_varint_list(writer, &files_per_block)?;

        // 3. Compressors
        let compressors: Vec<String> = self
            .data_blocks
            .iter()
            .map(|b| b.compressor.clone())
            .collect();
