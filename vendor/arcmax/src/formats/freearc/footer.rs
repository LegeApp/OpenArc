...
use crate::formats::freearc::utils::{read_stringz, read_varint, write_stringz, write_varint};
use anyhow::Result;
...
impl FooterBlock {
    ...
    pub fn write<W: Write>(&self, writer: &mut W, footer_desc_pos: u64) -> Result<()> {
        ...
        for block in &self.control_blocks {
            ...
            let pos = block
                .pos
                .expect("Block position must be set for writing footer");
            let rel_offset = footer_desc_pos
                .checked_sub(pos)
                .expect("Block position is after footer descriptor?");
