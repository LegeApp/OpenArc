...
pub fn grzip_decompress(input: &[u8], expected_size: usize) -> Result<Vec<u8>> {
    ...
    if input.len() < 28 {
        return Err(anyhow!(
            "GRZip input too small for header (need 28 bytes, got {})",
            input.len()
        ));
    }
    ...
    if result < 0 {
        return Err(anyhow!(
            "GRZip decompression failed with error code: {}",
            result
        ));
    }
    ...
    if actual_size <= output.len() {
        ...
        return Err(anyhow!(
            "GRZip decompression returned size larger than expected: {} > {}",
            actual_size,
            expected_size
        ));
    }
...
pub fn grzip_compress(input: &[u8], mode: i32) -> Result<Vec<u8>> {
    ...
    if input.len() > GRZ_MAX_BLOCK_SIZE {
        return Err(anyhow!(
            "GRZip input exceeds maximum block size of {} bytes",
            GRZ_MAX_BLOCK_SIZE
        ));
    }
    ...
    if result < 0 {
        return Err(anyhow!(
            "GRZip compression failed with error code: {}",
            result
        ));
    }
