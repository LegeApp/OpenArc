    ...
    pub fn from_string(method: &str) -> Option<Self> {
        ...
        match method_lower.as_str() {
            ...
            s if s.starts_with("lzp:") => {
                ...
                if first_param.ends_with('m')
                    || first_param.ends_with('k')
                    || first_param.ends_with('g')
                {
...
impl LzpProcessor {
    ...
    fn apply_lzp_reverse_ffi(&self, data: &mut Vec<u8>, hash_size: u32) -> Result<()> {
        ...
        if result < 0 {
            return Err(anyhow!(
                "LZP decompression failed with error code: {}",
                result
            ));
        }
        ...
        if actual_size <= data.len() {
            ...
            return Err(anyhow!(
                "LZP decompression returned size larger than expected: {} > {}",
                actual_size,
                data.len()
            ));
        }
...
pub fn apply_lzp_post_processing(
    data: &mut Vec<u8>,
    method: &str,
...
pub fn lzp_compress(input: &[u8], min_match_len: i32, hash_size_log: i32) -> Result<Vec<u8>> {
    ...
    if result < 0 {
        return Err(anyhow!(
            "LZP compression failed with error code: {}",
            result
        ));
    }
