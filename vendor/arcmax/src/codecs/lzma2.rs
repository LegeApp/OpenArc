...
pub fn lzma2_decompress(
    ...
    if result < 0 {
        return Err(anyhow!(
            "LZMA2 decompression failed with error code: {}",
            result
        ));
    }
    ...
    if actual_size <= output.len() {
        ...
        return Err(anyhow!(
            "LZMA2 decompression returned size larger than expected: {} > {}",
            actual_size,
            expected_size
        ));
    }
...
pub fn lzma2_compress(
    ...
    if result < 0 {
        return Err(anyhow!(
            "LZMA2 compression failed with error code: {}",
            result
        ));
    }
    ...
    if actual_size <= output.len() {
        ...
        return Err(anyhow!(
            "LZMA2 compression returned size larger than buffer: {} > {}",
            actual_size,
            max_output_size
        ));
    }
