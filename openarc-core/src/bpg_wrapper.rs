...
pub fn encode_image_to_bpg<P: AsRef<Path>>(input_path: P, output_path: P, config: &BpgConfig) -> Result<()> {
    ...
    #[cfg(feature = "heif")]
    if is_heic {
        let mut decoder = HeicDecoder::new().context("Failed to create HEIC decoder")?;
    ...
    if is_heic {
        return Err(anyhow::anyhow!("HEIC support not enabled - rebuild with 'heif' feature"));
    }
