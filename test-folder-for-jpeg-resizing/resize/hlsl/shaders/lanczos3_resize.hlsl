// Lanczos-3 resize compute shader
// Implements high-quality Lanczos filtering with support radius of 3
...
#define LANCZOS_SIZE 3

// Lanczos filter kernel (support = 3)
float lanczos_filter(float x) {
    x = abs(x);
    if (x < 1e-6f) {
        return 1.0f;
    }
    ...
    float px = PI * x;
    float px_over_a = px / LANCZOS_SIZE;
    return (sin(px) / px) * (sin(px_over_a) / px_over_a);
}
...
[numthreads(8, 8, 1)]  // Smaller thread group due to higher computational load
void main(uint3 id : SV_DispatchThreadID) {
    ...
    for (int dy = -LANCZOS_SIZE + 1; dy <= LANCZOS_SIZE; dy++) {
        for (int dx = -LANCZOS_SIZE + 1; dx <= LANCZOS_SIZE; dx++) {
            int sample_x = center_x + dx;
            int sample_y = center_y + dy;

            float weight_x = lanczos_filter(src_x - sample_x);
            float weight_y = lanczos_filter(src_y - sample_y);
