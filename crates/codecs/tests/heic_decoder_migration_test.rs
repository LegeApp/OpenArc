...
mod heic_decoder_tests {
    ...
    fn test_decoder_creation() {
        ...
        assert!(codec.is_ok(), "Failed to create HEIC codec: {:?}", codec.err());
    }
    ...
    fn test_version_string() {
        ...
        assert!(ver_str.contains("heic-decoder-rs"), "Version string: {}", ver_str);
        assert!(ver_str.contains("pure Rust"), "Expected 'pure Rust' in version");
    }
    ...
    fn test_encoding_not_supported() {
        ...
        assert!(err_msg.contains("not supported") || err_msg.contains("decoding only"),
                "Expected encoding error, got: {}", err_msg);
    }
