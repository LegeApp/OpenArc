...
use windows::Win32::Devices::PortableDevices::{
    IPortableDeviceValues, PortableDeviceValues, WPD_CLIENT_MAJOR_VERSION,
    WPD_CLIENT_MINOR_VERSION, WPD_CLIENT_NAME, WPD_CLIENT_REVISION,
    WPD_CLIENT_SECURITY_QUALITY_OF_SERVICE, WPD_CONTENT_TYPE_FOLDER, WPD_OBJECT_CONTENT_TYPE,
...
pub(crate) fn make_values_for_open_device(
    current_app_identifiers: &AppIdentifiers,
) -> crate::WindowsResult<IPortableDeviceValues> {
    ...
    unsafe {
        device_values.SetUnsignedIntegerValue(
            &WPD_CLIENT_MAJOR_VERSION as *const _,
            current_app_identifiers.app_major,
        )
    }?;
    unsafe {
        device_values.SetUnsignedIntegerValue(
            &WPD_CLIENT_MINOR_VERSION as *const _,
            current_app_identifiers.app_minor,
        )
    }?;
    unsafe {
        device_values.SetUnsignedIntegerValue(
            &WPD_CLIENT_REVISION as *const _,
            current_app_identifiers.app_patch,
        )
    }?;
    ...
    unsafe {
        device_values.SetUnsignedIntegerValue(
            &WPD_CLIENT_SECURITY_QUALITY_OF_SERVICE as *const _,
...
pub(crate) fn make_values_for_create_folder(
    parent_id: &U16CStr,
    folder_name: &OsStr,
) -> crate::WindowsResult<IPortableDeviceValues> {
    let device_values: IPortableDeviceValues =
    ...
    unsafe {
        device_values.SetGuidValue(
            &WPD_OBJECT_CONTENT_TYPE as *const _,
            &WPD_CONTENT_TYPE_FOLDER as *const _,
        )
    }?;
...
pub(crate) fn make_values_for_create_file(
    parent_id: &U16CStr,
    file_name: &OsStr,
    file_size: u64,
) -> crate::WindowsResult<IPortableDeviceValues> {
    ...
    unsafe {
        device_values.SetStringValue(
            ...
            PCWSTR::from_raw(pcwstr_file_name.as_ptr()),
        )
    }?;
