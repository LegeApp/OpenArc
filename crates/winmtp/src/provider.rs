...
use windows::Win32::System::Com::{
    CoCreateInstance, CoInitializeEx, CoTaskMemFree, CLSCTX_ALL, COINIT_DISABLE_OLE1DDE,
...
impl Provider {
    ...
    pub fn new() -> crate::WindowsResult<Self> {
        unsafe {
            let hr = CoInitializeEx(
                None,
                COINIT_MULTITHREADED | COINIT_DISABLE_OLE1DDE, // Setting this flag avoids some overhead associated with Object Linking and Embedding (OLE) 1.0, an obsolete technology. (see https://learn.microsoft.com/en-us/windows/win32/learnwin32/initializing-the-com-library)
            );
    ...
    pub fn enumerate_devices(&self) -> Result<Vec<BasicDevice>, MtpError> {
        ...
        unsafe {
            ...
            device_mgr.GetDevices(std::ptr::null_mut(), &mut dev_count as *mut _)?;
        }
        ...
        unsafe { device_mgr.GetDevices(dev_ids.as_mut_ptr(), &mut fetched_devices as *mut _) }?;
...
fn get_friendly_name(mgr: &IPortableDeviceManager, dev_id: PCWSTR) -> Result<String, MtpError> {
    ...
    unsafe { mgr.GetDeviceFriendlyName(dev_id, PWSTR::null(), &mut required_len as *mut u32) }?;
    ...
    unsafe { mgr.GetDeviceFriendlyName(dev_id, p_friendly_name, &mut retrieved_len as *mut u32) }?;
