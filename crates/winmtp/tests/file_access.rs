...
fn access_by_path(basic_device: &BasicDevice) {
    ...
    let object_by_path = root_obj
        .object_by_path(Path::new(r"Internal shared storage\Download\"))
        .unwrap();
    ...
    let object_by_path = root_obj.object_by_path(Path::new(
    ...
    let object_by_path = root_obj
        .object_by_path(Path::new(r"Internal shared storage\.."))
        .unwrap();
    assert_eq!(object_by_path.object_type(), ObjectType::FunctionalObject);
...
fn access_by_id(basic_device: &BasicDevice) {
    ...
    let root_obj = content.root().unwrap();
    let download_folder_by_path = root_obj
        .object_by_path(Path::new(r"Internal shared storage\Download\"))
        .unwrap();
    let download_folder_by_id = content
        .object_by_id(download_folder_by_path.id().to_ucstring())
        .unwrap();
    assert_eq!(
        download_folder_by_id.name(),
        &U16CString::from_str_truncate("Download")
    );
}
...
fn push_content(basic_device: &BasicDevice) {
    ...
    let test_folder_id = match download_folder.create_subfolder(OsStr::new("winmtp_test")) {
        ...
        Err(winmtp::error::CreateFolderError::AlreadyExists) => {
            let mut existing_folder = download_folder
                .object_by_path(Path::new("winmtp_test"))
                .unwrap();
            existing_folder.delete(true).unwrap();
            // and try again
            download_folder
                .create_subfolder(OsStr::new("winmtp_test"))
                .unwrap()
        }
        Err(err) => panic!("{}", err),
    };
    ...
    test_folder
        .push_file(Path::new(EXAMPLE_FILE), true)
        .unwrap();
}
