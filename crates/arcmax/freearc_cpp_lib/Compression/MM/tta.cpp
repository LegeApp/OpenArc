...
// malloc1d function is now defined inline in ttaenc.h to avoid linking issues
...
// Wrapper functions to match CALLBACK_FUNC signature
int readFILE_wrapper(const char *what, void *data, int size, void *auxdata) {
  return readFILE(data, size);
}

int writeFILE_wrapper(const char *what, void *data, int size, void *auxdata) {
  return writeFILE(data, size);
}
...
int readFILE(/*void* param,*/ void *buf, int size) {
  // FILE *fin = (FILE*)param;
  int res = read(fileno(fin), buf, size);
  if (res > 0)
...
int main(int argc, char **argv) {
  ...
  !unpack ? tta_compress(tta_level, skip_header, is_float, num_chan, word_size,
                         offset, raw_data, readFILE_wrapper, fin)
          : tta_decompress(readFILE_wrapper, fin);
