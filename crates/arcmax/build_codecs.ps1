...
$targetDirName = if ($env:OPENARC_CODEC_TARGET_DIR) { $env:OPENARC_CODEC_TARGET_DIR } else { "windows-gnu" }
$stagingRoot = Join-Path $projectRoot "codec_staging"
$buildRoot = Join-Path $projectRoot "codec_build"
$stagingPath = Join-Path $stagingRoot $targetDirName
$buildPath = Join-Path $buildRoot $targetDirName
