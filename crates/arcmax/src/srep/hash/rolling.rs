...
impl PolyRolling64 {
    ...
    pub fn new(window: usize, prime: u64) -> Self {
        ...
        PolyRolling64 { value: 0, prime, prime_l, window }
    }
...
mod tests {
    ...
    // Verify that update() is consistent with move_to() by sliding one step.
    #[test]
    fn poly_update_consistent_with_moveto() {
        let window = 8usize;
        let data: Vec<u8> = (0u8..16).collect();

        let mut h = PolyRolling64::new(window, PRIME_M3);
        h.move_to(&data); // hash of data[0..8]

        // Manually compute expected hash of data[1..9]
        let mut h2 = PolyRolling64::new(window, PRIME_M3);
        h2.move_to(&data[1..]);

        h.update(data[0], data[window]);
        assert_eq!(h.value(), h2.value());
    }

    #[test]
    fn poly_zero_window_zero_hash() {
        let mut h = PolyRolling64::new(0, PRIME_M3);
        h.move_to(&[]);
        assert_eq!(h.value(), 0);
    }

    #[test]
    fn poly_sliding_window_regression() {
        // Slide across a 16-byte buffer and check that update() and move_to()
        ...
        for i in 1..=(data.len() - window) {
            h.update(data[i - 1], data[i + window - 1]);
            let mut expected = PolyRolling64::new(window, PRIME_M3);
            expected.move_to(&data[i..]);
            assert_eq!(h.value(), expected.value(), "mismatch at i={}", i);
        }
    }

    #[test]
    fn crc_update_consistent_with_moveto() {
        let window = 8usize;
        let data: Vec<u8> = (0u8..16).collect();

        let mut h = CrcRollingHash::new(window, CRC32_CASTAGNOLI);
        h.move_to(&data);

        let mut h2 = CrcRollingHash::new(window, CRC32_CASTAGNOLI);
        h2.move_to(&data[1..]);
    ...
    #[test]
    fn crc_sliding_window_regression() {
        ...
        for i in 1..=(data.len() - window) {
            h.update(data[i - 1], data[i + window - 1]);
            let mut expected = CrcRollingHash::new(window, CRC32_CASTAGNOLI);
            expected.move_to(&data[i..]);
            assert_eq!(h.value(), expected.value(), "mismatch at i={}", i);
        }
    }
    ...
    #[test]
    fn pow_wrapping_sanity() {
        assert_eq!(pow_wrapping(2, 10), 1024);
        assert_eq!(pow_wrapping(1, 1000), 1);
        assert_eq!(pow_wrapping(0, 5), 0);
        assert_eq!(pow_wrapping(u64::MAX, 2), 1); // (-1)^2 wraps to 1
    }

    #[test]
    fn fast_table_build_zero_entry() {
        // CRCTable[0] must always be 0 (any byte XOR'd with 0 carries nothing)
        let tab = fast_table_build(CRC32_CASTAGNOLI, CRC32_CASTAGNOLI);
        assert_eq!(tab[0], 0);
    }
}
