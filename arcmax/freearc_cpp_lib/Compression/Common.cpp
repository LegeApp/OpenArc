...
#ifdef _WIN32
// Windows doesn't have sys/resource.h, so define needed constants
...
#ifdef _WIN32
  // Windows equivalent - set thread priority to below normal
  SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_BELOW_NORMAL);
#else
  // Unix/Linux - use getpriority/setpriority
...
#ifdef _WIN32
  // Windows equivalent - get current thread priority and set to below normal
  int old = GetThreadPriority(GetCurrentThread());
  SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_BELOW_NORMAL);
  return old;
#else
  // Unix/Linux - use getpriority/setpriority
...
#ifdef _WIN32
  // Windows equivalent - restore thread priority
  SetThreadPriority(GetCurrentThread(), old_priority);
#else
  // Unix/Linux - restore priority using setpriority
...
double GetCPUTime(void) {
  ...
  if (GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime, &kernelTime, &userTime)) {
    ...
    return ((double)(user.QuadPart + kernel.QuadPart)) / 10000000.0;
  }
  return -1;
}
...
double GetThreadCPUTime(void) {
  ...
  if (GetThreadTimes(GetCurrentThread(), &creationTime, &exitTime, &kernelTime, &userTime)) {
    ...
    user.LowPart = userTime.dwLowDateTime;
    user.HighPart = userTime.dwHighDateTime;
    return ((double)user.QuadPart) / 10000000.0;
  }
  return -1;
}
#else
...
int systemRandomData(void *buf, int len) {
  ...
  NTSTATUS status = BCryptGenRandom(NULL, (PUCHAR)buf, len, BCRYPT_USE_SYSTEM_PREFERRED_RNG);
  #define NT_SUCCESS(Status) (((NTSTATUS)(Status)) >= 0)
  return NT_SUCCESS(status) ? len : 0;
#else
