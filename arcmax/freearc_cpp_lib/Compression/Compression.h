...
struct Buffer {
  ...
  void put8(uint x) {
    freearc_reserve(sizeof(uint8));
    *(uint8 *)p = x;
    p += sizeof(uint8);
  }
  void put16(uint x) {
    freearc_reserve(sizeof(uint16));
    setvalue16(p, x);
    p += sizeof(uint16);
  }
  void put32(uint x) {
    freearc_reserve(sizeof(uint32));
    setvalue32(p, x);
    p += sizeof(uint32);
  }
  ...
  void freearc_puts(char *s) { put(s, strlen(s)); }
  void freearc_putsz(char *s) { put(s, strlen(s) + 1); }

  int remainingSpace() { return bufend - p; }
  void freearc_reserve(uint n) {
    if (remainingSpace() < n) {
