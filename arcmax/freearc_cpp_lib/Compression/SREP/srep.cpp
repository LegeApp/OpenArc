...
// Global state for library mode
static jmp_buf srep_exit_jmp;
static int srep_exit_code = 0;
static srep_params_t* g_params = NULL;

// Wrapper functions for BigAlloc compatibility
static void* BigAllocWrapper(size_t size, LPType lp_mode) {
    return BigAlloc(size);
}

// Override BigAlloc macro
#undef BigAlloc
#define BigAlloc(size, lp_mode) BigAllocWrapper(size, lp_mode)

// Library-compatible error handling
void library_error(int code, const char* msg, ...) {
    srep_exit_code = code;
    if (g_params && g_params->log_cb) {
        va_list args;
        va_start(args, msg);
        char buf[1024];
        vsnprintf(buf, sizeof(buf), msg, args);
        g_params->log_cb(g_params->user_ctx, 0, buf);
        va_end(args);
    } else {
        va_list args;
        va_start(args, msg);
    ...
    longjmp(srep_exit_jmp, 1);
}

// Override exit() in library mode
#define exit(code) library_error(code, "Exit with code %d", code)

#else
// For standalone CLI mode, define LPType enum
typedef enum {
    TRY = 0,
    FORCE = 1,
    DISABLE = 2
...
#include "../_Encryption/hashes/vmac/vmac.c"
...
#ifdef SREP_LIB
int srep_main_impl(int argc, char **argv) {
#else
        ...
        NULL) // unless hash checking was disabled by -hash- option
    {
      ...
      if (selected_hash == NULL) {
        ...
        fprintf(stderr,
                ...
                hash_seed_size ? show3_impl(hash_seed_size * CHAR_BIT, temp, "-")
                               : "",
                hash_size * CHAR_BIT);
        selected_hash = hash_by_name("", errcode);
      }
...
int srep_init(srep_params_t* params) {
  if (!params) return SREP_ERROR_CMDLINE;
  g_params = params;
...
int srep_run(srep_params_t* params) {
  ...
  // Set up longjmp for error handling
  if (setjmp(srep_exit_jmp)) {
    return srep_exit_code;
  }
  
  // Convert library params to command line args format
  // This is a simplified conversion - in a full implementation,
  // you'd want to build argv array from params
  static char method_str[16];
  static char dict_str[32];
  static char buf_str[32];
  static char threads_str[16];
  static char accel_str[16];
  static char minmatch_str[16];
  static char L_str[16];
  
  snprintf(method_str, sizeof(method_str), "-m%d", params->method);
  snprintf(dict_str, sizeof(dict_str), "-d%zu", params->dict_size);
  snprintf(buf_str, sizeof(buf_str), "-b%zu", params->buf_size);
  snprintf(threads_str, sizeof(threads_str), "-t%d", params->num_threads);
  snprintf(accel_str, sizeof(accel_str), "-a%d", params->accel);
  snprintf(minmatch_str, sizeof(minmatch_str), "-l%u", params->min_match);
  snprintf(L_str, sizeof(L_str), "-c%u", params->L);
  ...
  // Call the original main implementation
  return srep_main_impl(argc, argv);
}

void srep_free(srep_params_t* params) {
  if (params) {
    params->internal_state = NULL;
  }
  g_params = NULL;
}
