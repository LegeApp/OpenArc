...
fn main() {
    ...
    // Ensure top-level binaries also pull in native BPG symbols.
    // The codecs crate links bpg_native internally, but static native links
    // do not always propagate to final binary link on MinGW.
    let target_os = env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
    link_workspace_native_libs(&manifest_dir, &target_os);
...
fn link_workspace_native_libs(manifest_dir: &str, target_os: &str) {
    if target_os != "windows" {
        return;
    }

    let libs_dir = PathBuf::from(manifest_dir).join("libs");
    let bpg_archive = libs_dir.join("libbpg_native.a");
    if libs_dir.exists() {
        println!("cargo:rustc-link-search=native={}", libs_dir.display());
        if bpg_archive.exists() {
            // Link the archive for the CLI binary with an explicit group so MinGW
            // can resolve transitive C/C++ deps in one pass.
            println!(
                "cargo:rustc-link-arg-bin=openarc=-Wl,--start-group"
            );
            println!(
