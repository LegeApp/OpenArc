...
$bpgDllPath = Join-Path $PSScriptRoot "BPG\libbpg-0.9.8\openarc_bpg.dll"
if (-not (Test-Path $bpgDllPath)) {
    throw "openarc_bpg.dll not found at $bpgDllPath. Build it first via BPG\libbpg-0.9.8\build_openarc_combined_dll.ps1."
}
...
if (-not $SkipFfmpegDllBuild) {
    ...
    try {
        & $gcc -shared -O2 -o "dist\openarc_ffmpeg.dll" "codecs\ffmpeg_wrapper.c" `
...
$searchDirs = @(
    ...
    (Join-Path $PSScriptRoot "BPG\libbpg-0.9.8"),
